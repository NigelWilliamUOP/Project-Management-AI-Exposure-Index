(function () {
  const data = window.PM_EXPOSURE_DATA;
  if (!data || !Array.isArray(data.rows)) {
    console.error("Missing comparison site data.");
    return;
  }
  ...
  function renderRows() {
    ...
    elements.resultCount.textContent = `${rows.length} systems shown`;
    elements.coverageSummary.textContent = `Average benchmark coverage in full snapshot: ${formatCoverage(
      data.summary.mean_benchmark_coverage_ratio
    )}`;
    ...
    rows.forEach((row) => {
      ...
      systemName.textContent = row.display_name;
      const systemId = document.createElement("span");
      systemId.className = "system-id";
      systemId.textContent = row.model_id;
      systemCell.appendChild(systemName);
      systemCell.appendChild(systemId);
      ...
      if (!benchmarkList.children.length) {
        benchmarkList.textContent = "No benchmark metadata";
      }
